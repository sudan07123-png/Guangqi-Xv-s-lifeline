...
const NarrativePage = (function() {
  ...
  function init() {
    ...
    // 素材缺失时静默隐藏立绘，避免出现浏览器「破图」图标
    if (charLeft) charLeft.addEventListener('error', function() { charLeft.classList.add('hidden'); charLeft.src = ''; });
    if (charRight) charRight.addEventListener('error', function() { charRight.classList.add('hidden'); charRight.src = ''; });
  ...
  function enter(params) {
    ...
    // 加载地图位置数据（用于场景标题的中文地名解析）
    if (!mapLocationsData) {
      DataLoader.loadMapLocations().then(function(data) {
        mapLocationsData = data;
      });
    }
  ...
  function _showSceneTitle(title, location) {
    ...
    _setLocationText(sceneTitleOverlay.querySelector('.scene-location'), location);
  ...
  /**
   * 将地点 ID（如 loc_shanghai）解析为中文地名（如 上海·徐家汇）
   */
  function _setLocationText(el, locId) {
    if (!el) return;
    if (!locId) { el.textContent = ''; return; }
  ...
  function _renderNarration(step) {
    ...
    currentSkip = Components.typewriter(textEl, plainText, Math.round(Settings.textSpeedMs() * 0.9), function() {
      currentSkip = null;
      // 打字机完成后应用引用样式
  ...
  function _renderDialogue(step) {
    ...
    currentSkip = Components.typewriter(textEl, plainText, Settings.textSpeedMs(), function() {
      currentSkip = null;
      _applyCitationAfterTypewriter(textEl, plainText, citationRef, citationAnchor);
  ...
  function _renderInner(step) {
    ...
    currentSkip = Components.typewriter(textEl, plainText, Math.round(Settings.textSpeedMs() * 1.1), function() {
      currentSkip = null;
      _applyCitationAfterTypewriter(textEl, plainText, citationRef, citationAnchor);
    });
